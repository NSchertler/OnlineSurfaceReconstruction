...
void ShaderPool::CompileAll()
{
	...
	{		
		MeshColorsTriShader.initWithTessellation("MeshColorsTriShader", std::string((const char*)void_vert, void_vert_size), std::string((const char*)mesh_colors_tri_tcs, mesh_colors_tri_tcs_size), std::string((const char*)mesh_colors_tri_tes, mesh_colors_tri_tes_size), std::string((const char*)blinnphong_frag, blinnphong_frag_size), std::string((const char*)flat_shading_geom, flat_shading_geom_size));
		MeshColorsQuadShader.initWithTessellation("MeshColorsQuadShader", std::string((const char*)void_vert, void_vert_size), std::string((const char*)mesh_colors_quad_tcs, mesh_colors_quad_tcs_size), std::string((const char*)mesh_colors_quad_tes, mesh_colors_quad_tes_size), std::string((const char*)blinnphong_frag, blinnphong_frag_size), std::string((const char*)flat_shading_geom, flat_shading_geom_size));
