...
{
	hasMeshColorSupport = glfwExtensionSupported("GL_ARB_shader_storage_buffer_object") == GLFW_TRUE
		&& glfwExtensionSupported("GL_ARB_tessellation_shader") == GLFW_TRUE;
	...
	if (hasMeshColorSupport)
	{	
		hasMeshColorSupport = hasMeshColorSupport && MeshColorsTriShader.initWithTessellation("MeshColorsTriShader", std::string((const char*)void_vert, void_vert_size), std::string((const char*)mesh_colors_tri_tcs, mesh_colors_tri_tcs_size), std::string((const char*)mesh_colors_tri_tes, mesh_colors_tri_tes_size), std::string((const char*)blinnphong_frag, blinnphong_frag_size), std::string((const char*)flat_shading_geom, flat_shading_geom_size), true);
		hasMeshColorSupport = hasMeshColorSupport && MeshColorsQuadShader.initWithTessellation("MeshColorsQuadShader", std::string((const char*)void_vert, void_vert_size), std::string((const char*)mesh_colors_quad_tcs, mesh_colors_quad_tcs_size), std::string((const char*)mesh_colors_quad_tes, mesh_colors_quad_tes_size), std::string((const char*)blinnphong_frag, blinnphong_frag_size), std::string((const char*)flat_shading_geom, flat_shading_geom_size), true);
		hasMeshColorSupport = hasMeshColorSupport && TessellatedEdgesShader.initWithTessellation("EdgeShader", (const char*)edges_vert, std::string((const char*)edges_tcs, edges_tcs_size), std::string((const char*)edges_tes, edges_tes_size), std::string((const char*)lines_frag, lines_frag_size), "", true);
	}	
...
bool ShaderPool::HasMeshColorSupport() const
{
	return hasMeshColorSupport;
}