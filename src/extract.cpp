...
void
extract_graph(THierarchy& hierarchy, Float scale, bool extrinsic, IOrientationFieldTraits* rosy, IPositionFieldTraits* posy,
              ...
              bool snap_vertices) 
{
    ...
    Timer<> timer;
    {
		uint32_t size32 = (uint32_t)hierarchy.vertexCount();
		if (hierarchy.vertexCount() != size32)
			throw std::runtime_error("The utilized union-find data structure cannot handle " + std::to_string(hierarchy.vertexCount()) + " points.");

        DisjointSets dset(size32);
        adj_new.clear();
        adj_new.resize(hierarchy.vertexCount());
        typedef std::pair<Edge, float> WeightedEdge;

        tbb::concurrent_vector<WeightedEdge> collapse_edge_vec;
        collapse_edge_vec.reserve(hierarchy.vertexCount()*2.5f);
        ...
        auto classify_edges = [&](const tbb::blocked_range<uint32_t> &range)
		{
			for (uint32_t i = range.begin(); i != range.end(); ++i)
			{
				...
				auto current = hierarchy.fromConsecutiveIndex(i);
				Vector3f p = hierarchy.attribute<Position>(current);
				Vector3f o = hierarchy.attribute<PosField>(current);
				Vector3f q = hierarchy.attribute<DirField>(current);
				Vector3f n = hierarchy.attribute<Normal>(current);

				for (auto neighbor : hierarchy.neighbors(current))
				{
					size_t index = hierarchy.toConsecutiveIndex(neighbor);
					...
					Vector3f pn = hierarchy.attribute<Position>(neighbor);
					Vector3f on = hierarchy.attribute<PosField>(neighbor);
					Vector3f qn = hierarchy.attribute<DirField>(neighbor);
					Vector3f nn = hierarchy.attribute<Normal>(neighbor);

					std::pair<Vector3f, Vector3f> Q_rot = rosy->findCompatible(q, n, qn, nn);

					Float error = 0;
					std::pair<Vector2i, Vector2i> shift = posy->findCompatibleIndex(p, n, Q_rot.first, o, pn, nn, Q_rot.second, on, scale, inv_scale, &error);

					Vector2i absDiff = (shift.first - shift.second).cwiseAbs();

					if (absDiff.maxCoeff() > 1 || (absDiff == Vector2i(1, 1) && posy->posy() == 4))
        ...
        std::vector<uint16_t> nCollapses(hierarchy.vertexCount(), 0);
        ...
        std::cout << "Step 1: Classifying " /*<< nEdges << " edges "*/ << (deterministic ? "in parallel " : "") << ".. ";
        std::cout.flush();
        tbb::blocked_range<uint32_t> range1(0u, (uint32_t) hierarchy.vertexCount());
        if (!deterministic)
        ...
        std::cout << "Step 2: Collapsing " << collapse_edge_vec.size() << " edges .. ";
        std::cout.flush();
        ...
        std::cout << "done. (ignored " << nConflicts << " conflicting edges, took " << timeString(timer.reset()) << ")" << std::endl;

        std::cout << "Step 3: Assigning vertices .. ";
        std::cout.flush();
        ...
        std::cout << "done. (" << vertex_map.size() << " vertices, took " << timeString(timer.reset()) << ")" << std::endl;

        if (remove_spurious_vertices) {
            std::cout << "Step 3a: Removing spurious vertices .. ";
            std::cout.flush();
            uint32_t removed = 0;
            for (uint32_t i=0; i<adj_new.size(); ++i) {
            ...
            std::cout << "done. (removed " << removed << " vertices, took " << timeString(timer.reset()) << ")" << std::endl;
        }

        std::cout << "Step 4: Assigning positions to vertices .. ";
        std::cout.flush();
...

        {
            ...
            tbb::blocked_range<uint32_t> range(0u, hierarchy.vertexCount(), GRAIN_SIZE);
            ...
            auto map = [&](const tbb::blocked_range<uint32_t> &range) {
				for (uint32_t i = range.begin(); i < range.end(); ++i) {
					...
					Float weight = std::exp(-(hierarchy.attribute<PosField>(current) - hierarchy.attribute<Position>(current)).squaredNorm() * inv_scale * inv_scale * 9);
                    ...
                    for (uint32_t k=0; k<3; ++k) {
                        atomicAdd(&O_new.coeffRef(k, j), hierarchy.attribute<PosField>(current)(k)*weight);
                        atomicAdd(&N_new.coeffRef(k, j), hierarchy.attribute<Normal>(current)(k)*weight);
                    }
    ...
    if (remove_unnecessary_edges) {
        std::cout << "Step 5: Snapping and removing unnecessary edges .";
        std::cout.flush();
        bool changed;
        ...
        std::cout << " done. (snapped " << nSnapped << " vertices, removed " << nRemoved << " edges, took " << timeString(timer.reset()) << ")" << std::endl;
    }

    std::cout << "Step 6: Orienting edges .. ";
    std::cout.flush();
...
void extract_faces(std::vector<std::vector<TaggedLink> > &adj, MatrixXf &posField,
    ...
    uint32_t nF = 0, nV = posField.cols(), nV_old = posField.cols();
    F.resize(posy, posy == 4 ? posField.cols() : posField.cols()*2);
            ...
            if (verts.size() == (size_t) posy) {
                ...
                for (uint32_t i=0; i<verts.size(); ++i) {
                    ...
                    for (int k = 0; k < posy; ++k) {
                        Vector3f v0 = posField.col(verts[(i+k)%verts.size()].first);
                        Vector3f v1 = posField.col(verts[(i+k+1)%verts.size()].first);
                        Vector3f v2 = posField.col(verts[(i+k+2)%verts.size()].first);
                ...
                for (uint32_t k=0; k<verts.size(); ++k) {
                    centroid += posField.col(verts[k].first);
                    centroid_normal += N.col(verts[k].first);
                }
                uint32_t idx_centroid = nV++;
                if (nV > posField.cols()) {
                    posField.conservativeResize(posField.rows(), posField.cols() * 2);
                    N.conservativeResize(posField.rows(), posField.cols());
                }
                posField.col(idx_centroid) = centroid / verts.size();
    ...
    std::cout << "Step 7: Extracting faces .. ";
    std::cout.flush();
    ...
    std::cout << "done. (" << nFaces << " faces, took " << timeString(timer.reset()) << ")" << std::endl;

    if (fill_holes) {
        std::cout << "Step 8: Filling holes .. ";
        std::cout.flush();
        for (uint32_t i=0; i<nV_old; ++i) {
            for (uint32_t j=0; j<adj[i].size(); ++j) {
                if (!adj[i][j].used()) {
        ...
        std::cout << "done. (" << nHoles << " holes, took " << timeString(timer.reset()) << ")" << std::endl;
    }
    ...
    /*if (posy == 4 && pure_quad) {
        ...
        posField.conservativeResize(3, nV);
        VectorXu V2E, E2E, E2E_v;
        VectorXb nonManifold, boundary;
        build_dedge(F, posField, V2E, E2E, boundary, nonManifold, ProgressCallback(), true);
        E2E_v.resize(E2E.size());
        E2E_v.setConstant((uint32_t) -1);
        uint32_t nF_old = nF;
        std::cout << "Step 9: Regular subdivision into pure quad mesh .. ";
        std::cout.flush();
        ...
        for (uint32_t i=0; i<nF_old; ++i) {
            ...
            if (face[2] == face[3])
                continue;
            Vector3f fc = posField.col(face[0]) + posField.col(face[1]) + posField.col(face[2]) + posField.col(face[3]);
            ...
            posField.col(idx_fc) = fc * 1.f/4.f;
            ...
            for (uint32_t j=0; j<4; ++j) {
                ...
                if (idx_ec == INVALID) {
                    ...
                    posField.col(idx_ec) = (posField.col(i0) + posField.col(i1)) * 0.5f;
    ...
    //    std::cout << "Step 10: Running " << smooth_iterations << " smoothing & reprojection steps ..";
    //    std::cout.flush();
    ...
    //        MatrixXf O_prime(posField.rows(), posField.cols());
    //        MatrixXf N_prime(posField.rows(), posField.cols());
    //        std::cout << ".";
    //        std::cout.flush();

    //        tbb::parallel_for(
    //            tbb::blocked_range<uint32_t>(0u, (uint32_t) posField.cols(), GRAIN_SIZE),
    //            [&](const tbb::blocked_range<uint32_t> &range) {
    //                std::set<uint32_t> temp;
    //                for (uint32_t i = range.begin(); i != range.end(); ++i) {
    ...
    //                        for (auto j : adj_new[i])
    //                            cov += (posField.col(j)-centroid) * (posField.col(j)-centroid).transpose();
    ...
    //                        N_prime.col(i) = n;
    //                    } else {
    //                        O_prime.col(i) = posField.col(i);
    //                        N_prime.col(i) = N.col(i);
    //                    }
    //                }
    //            }
    //        );
    //        O_prime.swap(posField);
    //        N_prime.swap(N);
    //    }
    //    std::cout << " done. (took " << timeString(timer.reset()) << ")" << std::endl;
    //}
    ...
    for (uint32_t i=0; i < F.cols(); ++i) {
        ...
        for (int j=0; j<F.rows(); ++j) {
            uint32_t k = F(j, i);
            cov += (posField.col(k)-centroid) * (posField.col(k)-centroid).transpose();
        }
        Vector3f n = cov.jacobiSvd(Eigen::ComputeFullU).matrixU().col(2).normalized();
    ...
    remove_nonmanifold(F, posField, Nf);
    std::cout << "done. (took " << timeString(timer.reset()) << ")" << std::endl;
#endif

    /*std::cout << "Step 12: Reordering mesh for efficient access .. ";
    std::cout.flush();
    std::vector<MatrixXf> V_vec(2), F_vec(1);
    V_vec[0].swap(posField);
    V_vec[1].swap(N);
    F_vec[0].swap(Nf);
